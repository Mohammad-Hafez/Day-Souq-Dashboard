...
export default function AddProduct({ LoaderBtn,headers, displayAddNewDialog,sec ,AllProducts, secName , secId ,SecProductsRefetch, brandsNameResponse, categoriesNameResponse, SubcategoriesNameResponse,subSubcategoriesNameResponse, hideDialog, setLoaderBtn , AllRefetch }) {
...
const minDate = new Date();
  ...
  const getCatForSub = SubcategoriesNameResponse?.filter(subcategory => subcategory?._id === secId )
  ...
  const validationSchema = Yup.object().shape({
    ...
    ...((secName?.toLowerCase() === 'auction' || getCatForSub[0]?.name?.toLowerCase()=== 'auction')&& {
      ...
      biddingPrice: Yup.number().required('Biddingprice is required'),
  ...
  const AddNewProducts =  (values) => {
    ...
    formData.append('number_quantity', values.number_quantity ?values.number_quantity : 0);
    ...
    <>
      <Dialog header={'Add New Product'} className='container editDialog' visible={displayAddNewDialog} onHide={hideDialog} modal>
        <form onSubmit={AddNewFormik.handleSubmit} className='bg-light p-3 border shadow-sm rounded'>
          ...
          <div className="form-floating mb-2">
            <input type="text"placeholder='sku' className="form-control"id="sku"name="sku" minLength={10} maxLength={11} value={AddNewFormik.values.sku} onChange={AddNewFormik.handleChange} onBlur={AddNewFormik.handleBlur} />
          ...
          {(secName?.toLowerCase() === 'auction' || getCatForSub[0]?.name?.toLowerCase()=== 'auction' || selectedCategoryId==='6517dbc538001813b052bd73') && (
            <>
              ...
              <div className="form-floating mb-2">
                ...
                <label className='ms-2' htmlFor="biddingPrice">biddingprice</label>
